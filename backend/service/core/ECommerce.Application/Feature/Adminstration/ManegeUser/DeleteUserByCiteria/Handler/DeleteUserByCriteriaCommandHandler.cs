...
namespace ECommerce.Application.Feature.Adminstration.ManegeUser.DeleteUserByCiteria.Handler
{
    public sealed class DeleteUserByCriteriaCommandHandler : IRequestHandler<DeleteUserByCriteriaCommand,Result<UserDto>>
    {
        private readonly IUnitOfWork _unitOfWork;

        public DeleteUserByCriteriaCommandHandler(IUnitOfWork unitOfWork)
        {
        ...
        public async Task<Result<UserDto>> Handle(DeleteUserByCriteriaCommand command, CancellationToken ct)
        {
            try
            {
                ...
                {
                    return Result<UserDto>.Failure(Error.NotFound("User.NotFound", "The user you are trying to delete does not exist."));
                }

                targetUser.MarkAsDelete();
