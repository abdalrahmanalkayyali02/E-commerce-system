...
namespace ECommerce.Application.Feature.userMangement.User.Create.Handler
{
    public class CreateCustomerCommandHandler : IRequestHandler<CreateCustomerCommand, Result<CreateUserResponse>>
    {
        ...
        public async Task<Result<CreateUserResponse>> Handle(CreateCustomerCommand command, CancellationToken cancellationToken)
        {
            ...
            var results = new dynamic[] { emailResult, phoneResult, dobResult, fNameResult, lNameResult, uNameResult, passResult };
            var firstFailure = results.FirstOrDefault(r => r.IsError);
            if (firstFailure is not null) return Result<CreateUserResponse>.Failure(firstFailure.Errors);

            try
            {
                if (await _unitOfWork.Users.GetUserByEmail(emailResult.Value.Value, cancellationToken) is not null)
                    return Result<CreateUserResponse>.Failure(EmailAppError.Unique);

                if (await _unitOfWork.Users.GetUserByPhoneNumber(phoneResult.Value.Value, cancellationToken) is not null)
                    return Result<CreateUserResponse>.Failure(PhoneAppError.Unique);

                if (await _unitOfWork.Users.GetUserByUserName(uNameResult.Value.Value, cancellationToken) is not null)
